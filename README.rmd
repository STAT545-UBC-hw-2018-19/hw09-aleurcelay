...
* :star2:[Makefile](Makefile): I modified the Makefile to add the new files to the pipeline that generate the new plot, the new report, the modified README, and to create subdirectories to store the images (plots), the data outputs (csv files) and the source data (words.txt file).
You can test it by downloading this repo as a ZIP and then running `make clean` and `make all`. 
